...
(cl-macrolet ((color (name) (intern (concat "hannover-night-color-" (symbol-name name))))
  ...
  (cl-flet ((box (height color)
      ...
      (custom-theme-set-faces
       ...
       `(highlight ((t (:underline (:color foreground-color :style line) :foreground ,(color yellow)))))
       ...
       `(diff-header ((t (:background ,(color bg-highlight)))))
       `(diff-file-header ((t (:background ,(color bg-highlight) :foreground ,(color fl-keyword)))))
       `(diff-function ((t (:inherit diff-header))))
       `(diff-hunk-header ((t (:inherit diff-header :foreground ,(color fl-type) :weight bold))))
       ...
       `(anzu-replace-to ((t (:foreground ,(color red)))))
       ...
       `(company-scrollbar-bg ((t (:background ,(mix (color fg-tooltip) (color bg-tooltip) 0.9)))))
       `(company-scrollbar-fg ((t (:background ,(mix (color fg-tooltip) (color bg-tooltip) 0.7)))))
